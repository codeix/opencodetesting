...
description: >
  ...
  text (and, rarely, a cropped screenshot). Use this FIRST for any "new test",
  "generate a page object for X", or "explore this page" request, before
  generate-pageobject — never guess a page's structure from memory or from HTML you
  weren't given. Also use for a small, targeted re-check when edit-test needs one new
  selector that isn't already known.
---
...
5. Pass the ARIA snapshot text straight to `generate-pageobject` — do not summarize,
   reformat, or drop structural detail from it.
